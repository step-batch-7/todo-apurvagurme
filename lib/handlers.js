...
const serveNewTodo = function(req, res) {
  let id = 0;
  if (todoList.todoRecords[0]) {
    id = todoList.todoRecords[0].id;
  }
  const todo = Todo.getTodo(req.body.title, id);
...
const sendResAfterChange = function(res) {
  const records = JSON.stringify(todoList);
...
const toggleTaskStatus = function(req, res) {
  todoList.getMatchedIdTodo(req.body.id, req.body.todoId);
  sendResAfterChange(res);
};

const deleteSubtask = function(req, res) {
  todoList.getIdOfDeletedTodo(req.body.id, req.body.todoId);
  sendResAfterChange(res);
};
...
const saveNewSubtask = function(req, res) {
  ...
  todoList.changeEditedSubTask(subtaskId, subtask, todoId);
  sendResAfterChange(res);
};
...
app.post('/deleteTask', deleteSubtask);
app.post('/deleteTodo', deleteTodo);
app.post('/addTodo', serveNewTodo);
app.post('/addTask', addSubtask);
app.post('/renameTodo', saveNewTitle);
app.post('/renameTask', saveNewSubtask);
app.post('/searchTodo', serveRequiredTodo);
app.post('/searchTask', serveRequiredSubtask);
// post not found
