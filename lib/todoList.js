...
class TodoList {
  ...
  renameTask(taskId, newTaskName, todoId) {
    const matchedTodo = this.getMatchedTodo(todoId);
    return Boolean(matchedTodo) && matchedTodo.renameTask(taskId, newTaskName);
  }

  toggleTaskStatus(taskId, todoId) {
    const matchedTodo = this.getMatchedTodo(todoId);
    return Boolean(matchedTodo) && matchedTodo.toggleTaskStatus(taskId);
  }

  deleteTask(taskId, todoId) {
    const matchedTodo = this.getMatchedTodo(todoId);
    return Boolean(matchedTodo) && matchedTodo.deleteTask(taskId);
  }
